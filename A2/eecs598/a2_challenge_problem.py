...
# Datasets live in the shared top-level `datasets/` folder (this file is at
# <repo>/A2/eecs598/a2_challenge_problem.py), keeping MNIST in the same place
# as CIFAR10 regardless of the current working directory.
_DATASETS_ROOT = os.path.join(os.path.dirname(__file__), "..", "..", "datasets")
...
def evaluate_MNIST(w1, w2):
    ...
    MNIST_test_set = torchvision.datasets.MNIST(
        _DATASETS_ROOT, train=False, download=True, transform=transform
    )
...
def visualize_MNIST():
    ...
    MNIST_train_set = torchvision.datasets.MNIST(
        _DATASETS_ROOT, train=True, download=True, transform=transform
    )
