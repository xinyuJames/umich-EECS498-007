...
# Shared dataset directory: the folder that contains this file. Each
# assignment's eecs598/__init__.py loads this module (via importlib) and
# exposes it as `eecs598.data`, so CIFAR10 is always downloaded to / loaded
# from this `datasets/` folder regardless of the current working directory.
# os.path.realpath is used so the path is correct even if this file is
...
def cifar10(num_train=None, num_test=None, x_dtype=torch.float32):
    ...
    """
    download = not os.path.isdir(os.path.join(DATA_ROOT, "cifar-10-batches-py"))
    dset_train = CIFAR10(root=DATA_ROOT, download=download, train=True)
    dset_test = CIFAR10(root=DATA_ROOT, train=False)
