...
def compute_distances_two_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    for i in range(num_test):
        for j in range(num_train):
            dist = torch.sqrt(((test_flat[i] - train_flat[j]) ** 2).sum())
            dists[j, i] = dist
...
def compute_distances_one_loop(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    for i in range(num_test):
        dist = torch.sqrt(((train_flat - test_flat[i].unsqueeze(dim=0))**2).sum(dim=1))
        dists[:, i] = dist
...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    # use formula |a-b|^2 = a^2 + b^2 - 2 * a@b
    test_flat = x_test.reshape(num_test, -1)
    train_flat = x_train.reshape(num_train, -1)

    dists = ((train_flat**2).sum(dim=1).unsqueeze(dim=1) \
        + (test_flat**2).sum(dim=1).unsqueeze(dim=0) \
        - 2 * torch.mm(train_flat, test_flat.t())).sqrt()
...
def predict_labels(dists: torch.Tensor, y_train: torch.Tensor, k: int = 1):
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    _, sel_idx = torch.topk(input=dists, k=k, dim=0, largest=False)
    # print(sel_idx[:, 1])
    for i in range(sel_idx.shape[1]):
        most_freq, _ = torch.mode(y_train[sel_idx[:, i]])
        # print(y_train[sel_idx[i]])
        y_pred[i] = most_freq
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
...
def knn_cross_validate(
    ...
    for _k in k_choices:
        ...
        for i in range(num_folds):
            ...
            o = classifier.check_accuracy(x_test_fold, y_test_fold, k=_k, quiet=True)
            k_to_accuracies[_k].append(o)
