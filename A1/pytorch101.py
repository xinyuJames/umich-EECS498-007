...
def multiples_of_ten(start: int, stop: int) -> Tensor:
    ...
    for i in range(first_cat, stop, 10):
        x.append(i)
...
def slice_indexing_practice(x: Tensor) -> Tuple[Tensor, Tensor, Tensor, Tensor]:
    ...
    # Replace "pass" statement with your code
    last_row = x[-1, :]
    third_col = x[:, 2:3] # use slice, not index, which collapse that dim
...
def reverse_rows(x: Tensor) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    idx = torch.arange(x.shape[0]).flip(0)
    y = x[idx, :]
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
